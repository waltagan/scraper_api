...
Cada gateway recebe 2 sessões com fingerprints TLS distintos (chrome131, chrome124)
para diversidade mínima. libcurl gerencia internamente:
  - TCP connection keepalive (reutiliza conexão aberta)
  - TLS session resumption (evita re-negotiation)
  - DNS cache (default 60s TTL por handle)
"""
...
_IMPERSONATE = ["chrome131", "chrome124"]
_MAX_CLIENTS = 1000
...
async def get_session(proxy_url: str) -> "AsyncSession":
    ...
    Rotaciona entre impersonate profiles para diversidade de fingerprint TLS.
    """
    ...
    async with _get_lock():
        ...
        logger.info(
            ...
            f"(max_clients={_MAX_CLIENTS}/session, pool_total={total})"
        )
