"""Stress test Global + HK gateways — Sites BR Reais (somente texto)."""
...
async def run_test(gw_name, proxy_url, conc):
    ...
    log(f"{gw_name:<8} | conc={conc:>4} | {success}/{n} ({ok_pct:.1f}%) | {dur:.1f}s | RPS={n/dur:.1f}")
    if lats:
...
async def main():
    log("=" * 70)
    log("STRESS TEST GLOBAL + HK — Sites BR Reais (somente texto)")
    log("=" * 70)

    for gw_name, proxy_url in GATEWAYS.items():
        log(f"\n--- {gw_name} ---")
        log("Validação (1 request)...")
        await run_test(gw_name, proxy_url, 1)
        log("")
        for c in [50, 200, 500, 800]:
            await run_test(gw_name, proxy_url, c)
