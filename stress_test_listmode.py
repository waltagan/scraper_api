#!/usr/bin/env python3
"""
Stress test — List Mode 711Proxy.

Extrai IPs via API, depois dispara HEAD requests usando esses IPs como proxy.
Compara com o gateway mode para encontrar diferenças de performance.
"""
...
API_PARAMS = "zone=custom&ptype=1&region=BR&proto=http&stype=text&split=%0A&sessType=rotating"
...
async def fetch_proxies(count: int) -> List[str]:
    """Busca N proxy IPs via API do 711Proxy."""
    proxies = set()
    calls_needed = (count + 899) // 900

    async with aiohttp.ClientSession() as session:
        for i in range(calls_needed):
            batch = min(900, count - len(proxies))
            url = f"{API_BASE}?{API_PARAMS}&count={batch}"
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                ...
                for line in text.strip().split("\n"):
                    ...
                    if line and ":" in line:
                        proxies.add(line)
            print(f"    API call {i+1}/{calls_needed}: {len(proxies)} IPs únicos até agora")
...
async def run_test(concurrency: int, proxy_urls: List[str]) -> TestResult:
    """Dispara C requests HEAD simultâneos distribuídos entre os proxies."""
    ...
    async with aiohttp.ClientSession(
        ...
        print(f"\n  C={concurrency:>5d} | {len(proxy_urls)} proxy(s) | disparando... ", end="", flush=True)
        start = time.perf_counter()

        tasks = []
        for i in range(concurrency):
            proxy = proxy_urls[i % len(proxy_urls)]
            tasks.append(_single_request(session, TEST_URL, proxy))
...
async def main():
    ...
    print("=" * 80)
    print("  STRESS TEST — List Mode 711Proxy")
    ...
    max_needed = max(levels)
    print(f"\n  Buscando proxies via API ({max_needed} solicitados, max 900/chamada)...")
    proxy_ips = await fetch_proxies(max_needed)
    unique_hosts = set(ip.split(":")[0] for ip in proxy_ips)
    print(f"  Total: {len(proxy_ips)} IPs, {len(unique_hosts)} hosts únicos: {unique_hosts}")

    proxy_urls = [f"http://{ip}" for ip in proxy_ips]

    all_results: List[TestResult] = []

    for conc in levels:
        result = await run_test(conc, proxy_urls)
        all_results.append(result)
        await asyncio.sleep(3)

    print(f"\n{'=' * 80}")
    print("  RESUMO — LIST MODE")
    print(f"{'=' * 80}")
    ...
    print(f"\n  Comparação com Gateway Mode:")
    print(f"  (execute stress_test_gateway.py para comparar lado a lado)")
