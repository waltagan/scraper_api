"""
Stress test do proxy Decodo com URLs reais do banco de dados.
...
async def make_request(
    ...
    async with semaphore:
        ...
        try:
            ...
            elif "407" in err or "auth" in err:
...
async def run_level(
    ...
    for i, url in enumerate(test_urls):
        ...
        tasks.append(tracked_request(url, proxy))
...
def generate_markdown(all_results: List[Dict[str, Any]], proxy_label: str) -> str:
    lines = [
        "# Stress Test Proxy Decodo — Resultados",
        "",
        f"**Data:** {time.strftime('%Y-%m-%d %H:%M:%S')}",
        f"**Proxy:** {proxy_label}",
        f"**Timeout:** {TIMEOUT_SECONDS}s",
        f"**Níveis testados:** {', '.join(str(r['concurrency']) for r in all_results)}",
        f"**URLs:** reais do banco PostgreSQL (empresas com scrape OK)",
        "",
        "---",
        "",
        "## Resumo Comparativo",
    ...
    lines += [
        ...
        "## Breakdown de Erros",
        "",
        "| Conc. | timeout | connection | dns | ssl | reset | proxy_auth | http_4xx | http_5xx | other |",
        "|-------|---------|------------|-----|-----|-------|------------|----------|----------|-------|",
    ]
    for r in all_results:
        ...
        lines.append(
            ...
            f"| {eb.get('reset', 0)} | {eb.get('proxy_auth', 0)} "
...
async def main():
    ...
    for level in CONCURRENCY_LEVELS:
        result = await run_level(level, urls, proxy_urls)
        all_results.append(result)
        log("Aguardando 10s para proxy estabilizar...")
        await asyncio.sleep(10)

    proxy_label = f"Decodo br.decodo.com ({len(proxy_urls)} portas)"
    md = generate_markdown(all_results, proxy_label)
    output_file = "stress_test_decodo_results.md"
    ...
    json_file = "stress_test_decodo_results.json"
